function setUserFileStorage(data, type="full") 
{
	...
	var uploadedFileSize = data.log_info.used_file_size.toFixed(3)		// 현재까지 업로드한 파일 크기
	var limitFileSize = data.group_info.limit_file_size		// 업로드 한계 크기
	
	if (type == "full")
	{
		if (userinfo_limitfilesize > 0)
		{
			limitFileSize = data.group_info.limit_file_size.toFixed(3)
		}
		...
		if (limitFileSize == 0) {
			...
			html += '<center><p style="color:black;">' + usageRateFileSize + 'MB / ' + limitFileSize + 'MB' + '(' + usageRateFileSize_rate + '% 사용중)</p></center>'
			html += '</div>'
		}
	}
	else
	{
		var uploadedFileSize = data.log_info.used_file_size.toFixed(2)		// 현재까지 업로드한 파일 크기
		var limitFileSize = data.group_info.limit_file_size		// 업로드 한계 크기
		
		if (userinfo_limitfilesize > 0)
		{
			limitFileSize = data.group_info.limit_file_size.toFixed(2)
		}
