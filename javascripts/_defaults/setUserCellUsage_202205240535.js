function setUserCellUsage(data, type="full") 
{
	...
	var usedCellSize = parseInt(data.log_info.used_cell_size)		// 30일동안 사용한 셀 크기
	var limitCellSize = data.group_info.limit_cell_size		// 셀 사용 리밋

	if (type == "full")
	{
		if (limitCellSize == 0) {
			html = '<center><p style="color:black;">' + numberWithCommas(usedCellSize) + ' 개 / 0 개' + '</p></center>'
			
		} else if (limitCellSize == -1) {
