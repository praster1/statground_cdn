function ajax_dataInfo(requestUUID, subRequestUUID)
{
	...
	$(".div_data_generate_tooLarge").hide();
	...
	$.ajax({
		...
		success:function(data){
			...
			for(key in data.file_copyright)
			{
				...
				html += "(<a href='" + data.file_copyright_link[key].content.toString() + "' target='_blank'>자세히 보기</a>)"
				html += "</p>"
			}
			...
			if (isEmpty(data.data.full) || data.data.full.toString() == "Too large")
			{
				$(".div_data_generate_tooLarge").show();
				$('#btn_data_generate').attr('disabled', true);				// data_list의 링크 생성 버튼 - 잠금
			}
			else
			{
				$('#btn_data_generate').attr('disabled', false);				// data_list의 링크 생성 버튼 - 오픈
			}
